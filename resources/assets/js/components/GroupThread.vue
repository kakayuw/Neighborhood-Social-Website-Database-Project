...
<el-container>
  <!-- TABLE START -->
  <el-table
    :data="tableData?tableData.filter(data => !search || (data.title && data.title.toLowerCase().includes(search.toLowerCase())) || (data.text && data.text.toLowerCase().includes(search.toLowerCase()))):[]"
    style="width: 100%"
    >
    <el-table-column
      label="Date"
    ...
    <el-table-column  align="right">
      ...
      <template slot-scope="scope">
        <el-button
          size="mini"
          @click="loadThreadMessages(scope.row.thdid, scope.row.title)">View</el-button>
      </template>
    </el-table-column>
    <el-table-column
      label=" "
      prop="">
        ...
        <el-button type="primary" icon="el-icon-plus" circle @click="initNewMessage()"></el-button>
      </template>
    </el-table-column>
  </el-table>
  <!-- TABLE END -->
  ...
  <div class="tag-group" style="margin-left:3%">
    <span class="tag-group__title">Members</span>
    <el-tag
      v-for="mb in thread.members"
      :key="mb.uid"
      type="warning"
      effect="plain"
  ...
  <hr>
    <el-button type="primary" style="margin-left:3%" @click="groupNewMessage()">Add new Message</el-button>
  <hr>
  <div class="scrollable">
    <el-timeline>
      <el-timeline-item timestamp="2018/4/12" placement="top" v-for="m in thread.messages" :key="m.mid">
...
<script>
  ...
  export default {
    ...
    beforeCreate () {
      self = this;
      $.ajax({
          url: '/dbproj/public/getThreads',
          data: { type: "group"},
          type: "GET",
          dataType: "json",
          success:function (response) {
            self.tableData = response.threads;
    ...
    methods: {
      ...
      submitMessage() {
        ...
        $.ajax({
          url: self.is_init_msg?'/dbproj/public/initGroupMessage':'/dbproj/public/normalGroupMessage',
          data: {
            topic: title,
            text: self.init_msg.text,
            members: self.init_msg.members,
